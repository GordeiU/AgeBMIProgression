...
class Generator(nn.Module):
    ...
    def forward(self, out, age=None, bmi_group=None):
        logging.debug(f"forward:starting:age-{age}:bmi_group-{bmi_group}")
        ...
        logging.debug(f'forward:out.shape:{out.shape}')
        out = self.fc(out)
        logging.debug(f'forward:out.shape:{out.shape}')
...
class Net(object):
    def __init__(self):
        ...
        self.device = None
        self.cpu()  # initial, can later move to cuda or mps
    ...
    def kids(self, image_tensors, length, target):
        ...
        for i in range(length):
            ...
            fake_age = 0
            fake_bmi_group = random.choice([consts.MALE, consts.FEMALE])
    ...
    def test_single(self, image_tensor, age, bmi_group, target, watermark):
        ...
        self.eval()
        batch = image_tensor.repeat(consts.NUM_AGE_GROUPS, 1, 1, 1).to(device=self.device)  # N x D x H x W
        z = self.E(batch)  # N x Z

        bmi_group_tensor = -torch.ones(consts.NUM_BMI_GROUPS)
        bmi_group_tensor[int(bmi_group)] *= -1
        bmi_group_tensor = bmi_group_tensor.repeat(consts.NUM_AGE_GROUPS, consts.NUM_AGE_GROUPS // consts.NUM_BMI_GROUPS)  # apply bmi_group on all images

        age_tensor = -torch.ones(consts.NUM_AGE_GROUPS, consts.NUM_AGE_GROUPS)
        for i in range(consts.NUM_AGE_GROUPS):
        ...
        for epoch in range(1, epochs + 1):
            ...
            try:
                ...
                for i, (images, labels) in enumerate(train_loader, 1):
                    ...
                    logging.debug("iteration: "+str(i)+" images shape: "+str(images.shape))
                    z = self.E(images)


                    logging.debug("Input\Output Loss")
                    z_l = torch.cat((z, labels), 1)
                    generated = self.G(z_l)
                    eg_loss = input_output_loss(generated, images)
                    losses['eg'].append(eg_loss.item())

                    logging.debug("Total Variance Regularization Loss")
                    ...
                    logging.debug("DiscriminatorZ Loss")
                    z_prior = two_sided(torch.rand_like(z, device=self.device))  # [-1 : 1]
                    d_z_prior = self.Dz(z_prior)
                    d_z = self.Dz(z)
                    dz_loss_prior = binary_cross_entropy_with_logits(d_z_prior, torch.ones_like(d_z_prior))
                    dz_loss = binary_cross_entropy_with_logits(d_z, torch.zeros_like(d_z))
                    dz_loss_tot = (dz_loss + dz_loss_prior)
                    losses['dz'].append(dz_loss_tot.item())

                    logging.debug("Encoder\DiscriminatorZ Loss")
                    ez_loss = 0.0001 * binary_cross_entropy_with_logits(d_z, torch.ones_like(d_z))
                    ez_loss.to(self.device)
                    losses['ez'].append(ez_loss.item())

                    logging.debug("DiscriminatorImg Loss")
                    d_i_input = self.Dimg(images, labels, self.device)
                    d_i_output = self.Dimg(generated, labels, self.device)

                    di_input_loss = binary_cross_entropy_with_logits(d_i_input, torch.ones_like(d_i_input))
                    di_output_loss = binary_cross_entropy_with_logits(d_i_output, torch.zeros_like(d_i_output))
                    di_loss_tot = (di_input_loss + di_output_loss)
                    losses['di'].append(di_loss_tot.item())

                    logging.debug("Generator\DiscriminatorImg Loss")
                    dg_loss = 0.0001 * binary_cross_entropy_with_logits(d_i_output, torch.ones_like(d_i_output))
                    ...
                    logging.debug("Back prop on DiscriminatorImg")
